 ...
 */
class SM_XB2b_Model_Adminhtml_Search_Customer extends Varien_Object
{
...
//            }
            ...
            if ($address->getId()) {
                $data = array(
                    ...
                    'group_name' =>  Mage::getModel('customer/group')->load($customer->getData('group_id'))->getCode(),
...
//            if ($customerAddressId){
    ...
    public function loadById($customer_id){
        ...
        $collection = Mage::getResourceModel('customer/customer_collection')
            ...
            ->joinAttribute('street', 'customer_address/street', 'default_billing', null, 'left')
            ->addAttributeToFilter(array(
                array('attribute' => 'entity_id', 'eq' =>$customer_id)
            ))
            ->load();
